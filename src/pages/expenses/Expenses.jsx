...
function ExpenseFormModal({ expense, onClose, onSaved }) {
  ...
  // The API stores/returns the expense name in the `username` column
  const [name,    setName]    = useState(expense?.username ?? expense?.exp_name ?? expense?.name ?? '');
    ...
    <div className="exp-backdrop" onClick={onCancel}>
      <div className="exp-modal confirm" onClick={e => e.stopPropagation()}>
        ...
        <p className="confirm-msg">
          Remove <strong>"{expense.username || expense.exp_name || expense.name}"</strong> of{' '}
            ...
            <div className="exp-card" key={exp.id ?? i}>
              <div className="exp-card-left">
                ...
                <div className="exp-card-info">
                  <span className="exp-card-name">{exp.username || exp.exp_name || exp.name || '—'}</span>
